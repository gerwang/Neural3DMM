...
class MLPAE(nn.Module):
    def __init__(self, n_input, n_inter, nz):
        super().__init__()
        self.n_input = n_input
        self.n_inter = n_inter
        self.nz = nz
        self.en_fc1 = nn.Linear(n_input, n_inter)
        self.en_fc2 = nn.Linear(n_inter, nz)
        self.de_fc1 = nn.Linear(nz, n_inter)
        self.de_fc2 = nn.Linear(n_inter, n_input)
        self.activation = nn.ELU()
...
class MLPVAE(nn.Module):
    def __init__(self, n_input, n_inter, nz):
        ...
        self.nz = nz
        self.en_fc1 = nn.Linear(n_input, n_inter)
        self.en_mu = nn.Linear(n_inter, nz)
        self.en_logvar = nn.Linear(n_inter, nz)
        self.de_fc1 = nn.Linear(nz, n_inter)
        self.de_fc2 = nn.Linear(n_inter, n_input)
        self.activation = nn.ELU()
