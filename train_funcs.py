...
def train_variational_autoencoder_dataloader(dataloader_train, dataloader_val,
                                 device, model, optim, loss_fn, var_loss_fn, lambda_var,
                                 bsize, start_epoch, n_epochs, eval_freq, scheduler = None,
                                 writer=None, save_recons=True, shapedata = None,
                                 metadata_dir=None, samples_dir = None, checkpoint_path = None):
    if not shapedata.normalization:
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for b, sample_dict in enumerate(tqdm(dataloader_train)):
            ...
            data_loss = loss_fn(tx, tx_hat)
            var_loss = var_loss_fn(mu, logvar)

            loss = data_loss + lambda_var * var_loss
            loss.backward()
            optim.step()
            ...
            if writer and total_steps % eval_freq == 0:
                ...
                writer.add_scalar('loss/loss/var_loss', var_loss.item(), total_steps)
                writer.add_scalar('loss/loss/total_loss', loss.item(), total_steps)
                writer.add_scalar('training/learning_rate', optim.param_groups[0]['lr'],total_steps)
            total_steps += 1
